...
public class StudentsDAO {
	...
	public int delete(String scode) {
		...
		try {
			...
			System.out.println("학생 삭제 : " + e.toString());
		}
	...
	public void update(StudentsVO vo) {
		try {
			...
			System.out.println("학생 정보 수정 : " + e.toString());
		}
	}
	...
	public JSONObject list(SqlVO vo) {
		...
		try {
			...
			CallableStatement cs=Database.CON.prepareCall(sql);
			String tbl="(select s.*, pname from students s left join professors p on advisor=pcode) tbl";
			cs.setString(1, tbl);
			...
			while(rs.next()) {
				...
				//String strBirthday = sdf.format(rs.getDate("birthday"));
				//obj.put("birthday", strBirthday);
