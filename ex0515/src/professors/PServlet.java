...
@WebServlet(value= {"/pro/list", "/pro/cslist","/pro/insert", "/pro/delete", "/pro/read", "/pro/update"}) //웹 서블릿은 주소를 가려주는 역할을 한다. 여기서 벨류는 대체할 주소값을 미리 지정해주는 것 
public class PServlet extends HttpServlet { //import javax.servlet.ServletException 라리브러리를 약속어들을 사용하기위해 상속(extends)시킨다.
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {//doget(get을 실행하라) 리퀘스트와 리스폰스 값을 전달받아 오라는 뜻 
		...
		case "/pro/cslist":
			...
			break;
		case "/pro/read":
			pcode=request.getParameter("pcode"); //웹에서 입력된 pcode의 값을 가져와 변수 pcode에 입력함
			try {
				PVO vo=dao.read(pcode); //매개변수 pcode를 가지는 dao의 read메서드 실행값을 PVO vo에 치환
			...
			dis.forward(request, response); //리드jsp에 포워드(제어 요청)를 함, 포워드 값은 vo의 값을 가진다. 제어를 넘긴 값은 *리드jsp에서 처리후 바로 웹브라우저로 출력*
			break;
