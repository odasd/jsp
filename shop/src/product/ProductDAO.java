...
public class ProductDAO {
	...
	public ProductVO read(String prod_id) {
		...
		try {
			String sql = "select p.*, m.mall_name from product p, mall m where p.mall_id=m.mall_id and prod_id=?";
			PreparedStatement ps = Database.CON.prepareStatement(sql);
			...
			if (rs.next()) {
				...
				vo.setPrice1(rs.getInt("price1")); // 판매가
				vo.setPrice2(rs.getInt("price2")); // 일반가
	...
	public JSONObject list(SQLVO vo) {
		...
		try {
			...
			while (rs.next()) {
				...
				obj.put("price1", rs.getInt("price1")); // 판매가
				obj.put("price2", rs.getInt("price2")); // 일반가
